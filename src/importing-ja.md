...
Ankiは、テキストファイル、エクスポート機能で作成したAnkiデッキパッケージ、Mnemosyne 2.0の.dbファイル、SuperMemoの.xmlファイルをインポートできます。ファイルをインポートするには、メニューの［ファイル］→［インポート］を選択してください。
...
コンマ・セミコロン・タブのいずれかで区切られたフィールドがある**プレーンテキスト**ファイルは、Ankiにインポートできます。ただし、いくつかの条件を満たす必要があります。
...
- ファイルはプレーンテキスト (*.txt) である必要があります。他の形式、たとえば *.xls, *.rtf, *.doc の場合は、まずプレーンテキストとして保存する必要があります。

- 文字コードは UTF-8 を選択してください (後述)。

- Ankiは最初の行を読み取ってフィールドの区切り文字を判定します。たとえば、「；」（セミコロン）を読み取れば「；」になります。先に「,」（コンマ）を読み取れば「,」になります。（この区切り文字は、「インポート」ウィンドウで変更可能です。）

- フィールド数は、最初の（コメントではない）行を参照して決められます。
  - もし、他の行でフィールド数がそれよりも少ない場合は、その行で不足しているフィールドについては、内容が空のフィールドがあるものとして扱われます。
  - もし、他の行でフィールド数がそれよりも多い場合は、その行での余剰分のフィールドの内容はインポートされません。
