...
このように、波カッコの中に文字列を記入すると、Ankiはその文字列と同じ名前のフィールドを検索し、波カッコ付き文字列をそのフィールドの実際の内容に置き換えます。
...
フィールド名は大文字・小文字を区別します。例えば「Front」という名前のフィールドがある場合、{{front}} という書き方では正しく動作しません。
<!-- Field names are case sensitive. If you have a field named `Front`,
writing `{{front}}` will not work properly. -->

テンプレートに記載できるのは、フィールド名だけではありません。任意の文字列をテンプレートに記載することができます。例えば各国の首都を学習しているとして、「国」という名前のフィールドがあるノートを作成した場合、カード表面のテンプレートを次のようにすることができます：
...
テンプレート内の文字列と同様に、フィールド内の文字列もHTMLに格納されます。上記の辞書リンクの例では、式に書式指定なしで単語「myword」が含まれている場合、HTMLでも「myword」と記載されます。しかし、フィールド内に書式指定が含まれている場合、HTMLにその分の追加文字列が含まれます。例えば「myword」を太字にすると、実際のHTMLは「&lt;b&gt;myword&lt;/b&gt;」となります。
...
これは、辞書リンクなどにおいては問題となります。上記の例では、辞書リンクはこうなってしまいます：

    <a href="http://example.com/search?q=<b>myword</b>">check in dictionary</a>

リンク内の余分な文字列が辞書サイトを混乱させ、単語「myword」にマッチした検索結果を得られないでしょう。
...
この問題を解決するため、Ankiには文字列を置換する際にフィールドから書式指定を除去する機能が用意されています。
フィールド名の前に「text:」を付けると、書式指定を含ない文字列がHTMLに格納されるようになります。したがって、フィールド内に書式指定があっても機能する辞書リンクは次のようになります：
...
すべてのカードに同じ画像や音声を含めたい場合 (例：各カードの上部に会社のロゴを表示したい):
...
1. ファイル名を、アンダーバーで始まるように変更します（例："\_logo.jpg"）。このアンダーバーによって、「そのファイルがテンプレートで使用されており、デッキを共有する場合はそのファイルもエクスポートする必要がある」ことがAnkiに伝わります。
...
2. 表面または裏面のテンプレートに、メディアへの参照を追記します。下記はその一例です:
